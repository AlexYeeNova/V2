"""V2.4：连续采集 SHEIN 女装类目 Page1~Page5，并分别保存结果。"""
...
def build_v2_4_csv_path(page_number: int) -> Path:
    """生成 V2.4 指定页 CSV 保存路径。"""
    return OUTPUT_DIR / f"v2_4_products_page_{page_number}.csv"


def build_v2_4_debug_html_path(page_number: int) -> Path:
    """生成 V2.4 指定页调试 HTML 保存路径。"""
    return DEBUG_DIR / f"v2_4_page_{page_number}.html"
...
def save_error_debug_html(error: Exception, target_url: str, debug_html_path: Path) -> None:
    """当页面请求异常时，仍然落一份调试 HTML，避免五页输出缺文件。"""
    debug_html_path.parent.mkdir(parents=True, exist_ok=True)
    debug_html_path.write_text(
        "\n".join(
            [
                "<!doctype html>",
                "<html>",
                "<head><meta charset=\"utf-8\"><title>V2.4 fetch error</title></head>",
                "<body>",
                f"<h1>V2.4 页面采集异常</h1>",
...
def fetch_one_page(session: v23.StealthySession, page_number: int) -> PageResult:
    ...
    csv_path = build_v2_4_csv_path(page_number)
    debug_html_path = build_v2_4_debug_html_path(page_number)
...
def main() -> None:
    """执行 Page1~Page5 连续采集，单页异常后继续后续页。"""
