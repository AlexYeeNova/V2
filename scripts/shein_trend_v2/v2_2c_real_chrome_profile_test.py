"""V2.2A：采集 SHEIN 新加坡站女装类目第一页，并识别页面运行状态。"""
...
DEBUG_HTML_PATH = PROJECT_ROOT / "data" / "debug" / "v2_2c_page.html"
CSV_PATH = PROJECT_ROOT / "data" / "output" / "v2_2c_products_page_1.csv"
...
def fetch_page_with_real_chrome(session: StealthySession) -> Any:
...
def main() -> None:
    ...
    with StealthySession(
        ...
        page = fetch_page_with_real_chrome(session)
    ...
    if not products:
        raise RuntimeError("未采集到商品数据，请检查 data/debug/v2_2c_page.html 中的页面结构或拦截状态。")
