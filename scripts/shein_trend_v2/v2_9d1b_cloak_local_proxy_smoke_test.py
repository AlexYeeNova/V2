"""V2.9D-1B CloakBrowser Smoke Test：local CloakBrowser + KDL proxy detail collection."""
...
OUTPUT_CSV_PATH = PROJECT_ROOT / "data" / "output" / "v2_9d1b_cloak_smoke_detail_attributes.csv"
SUMMARY_CSV_PATH = PROJECT_ROOT / "data" / "output" / "v2_9d1b_cloak_smoke_summary.csv"
PREFLIGHT_MATRIX_CSV_PATH = PROJECT_ROOT / "data" / "output" / "v2_9d1b_preflight_matrix.csv"
DEBUG_DIR = PROJECT_ROOT / "data" / "debug" / "v2_9d1b_cloak_smoke"
MAX_PRODUCTS = 5
...
CAPTCHA_MANUAL_RESOLVE_ENABLED = True
...
LOW_RISK_SAMPLE_MODE = True
LOW_RISK_SKIP_TOP_N = 20
LOW_RISK_SAMPLE_LIMIT = 5
...
SUMMARY_FIELDS = [
    ...
    "cloak_humanize_enabled",
    "cloak_geoip_enabled",
    "low_risk_sample_mode",
    "low_risk_skip_top_n",
    "low_risk_sample_limit",
    "proxy_configured",
...
def create_cloak_proxy_browser_session() -> dict[str, Any]:
    """Launch CloakBrowser with the configured KDL proxy."""
    ...
    print("准备启动 CloakBrowser 浏览器实例...", flush=True)
    print("CloakBrowser 首次运行可能会下载专用 Chromium，请耐心等待。", flush=True)
    print("正在注入 KDL 代理配置...", flush=True)
    print("正在调用 cloak_launch() ...", flush=True)
    try:
        try:
            ...
            proxy_url = f"http://{PROXY_USERNAME}:{PROXY_PASSWORD}@{safe_server}"
            browser = cloak_launch(
                headless=False,
                proxy=proxy_url,
                humanize=CLOAK_HUMANIZE_ENABLED,
        ...
        print(f"CloakBrowser 启动失败：{type(exc).__name__}: {exc}", flush=True)
        print("如果这是首次运行，可能是 CloakBrowser 专用 Chromium 下载或初始化失败。", flush=True)
        raise

    print("cloak_launch() 调用完成，浏览器对象已创建。", flush=True)
    print("正在创建 CloakBrowser context...", flush=True)
    context = browser.new_context(
        locale="en-SG",
        timezone_id="Asia/Singapore",
        user_agent=USER_AGENT,
    ...
    print("CloakBrowser context 创建成功。", flush=True)
    print("正在创建 CloakBrowser page...", flush=True)
    page = context.new_page()
    print("CloakBrowser page 创建成功。", flush=True)

    if RESOURCE_BLOCKING_ENABLED:
        install_resource_blocking(page)

    print("CloakBrowser 本地代理 Session 创建完成。", flush=True)
    return {
        "browser": browser,
        "context": context,
        "page": page,
        "started_at": time.monotonic(),
        "success_count": 0,
        "session_type": "cloak_local_proxy",
    }
...
def select_sold_products(products: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    if LOW_RISK_SAMPLE_MODE:
        sample_end = LOW_RISK_SKIP_TOP_N + LOW_RISK_SAMPLE_LIMIT
        return candidates[LOW_RISK_SKIP_TOP_N:sample_end]
...
def open_browserleaks_before_collection(session: dict[str, Any]) -> None:
    """Open BrowserLeaks in the same CloakBrowser session before SHEIN collection."""
    ...
    try:
        ...
        print("BrowserLeaks 已在当前采集用 CloakBrowser session 中打开。")
        print("请人工检查 IP Address、WebRTC、DNS、IPv6、Timezone、User-Agent。")
        print("这是后续 SHEIN 采集使用的同一个 session/context。")
        print(f"浏览器将停留 {BROWSERLEAKS_OBSERVE_SECONDS} 秒，随后自动继续商品采集。")
        time.sleep(BROWSERLEAKS_OBSERVE_SECONDS)
    finally:
    ...
    row.update(
        {
            ...
            "captcha_recovery_success": "False",
            "recovery_used": "False",
...
def build_summary_row(
    ...
    session_type: str = "cloak_local_proxy",
) -> dict[str, str]:
    """生成 V2.9D 采集结果汇总行。"""
    ...
    return {
        ...
        "cloakbrowser_enabled": str(CLOAKBROWSER_ENABLED),
        "cloak_humanize_enabled": str(CLOAK_HUMANIZE_ENABLED),
        "cloak_geoip_enabled": str(CLOAK_GEOIP_ENABLED),
        "low_risk_sample_mode": str(LOW_RISK_SAMPLE_MODE),
        "low_risk_skip_top_n": str(LOW_RISK_SKIP_TOP_N),
        "low_risk_sample_limit": str(LOW_RISK_SAMPLE_LIMIT),
...
def wait_for_manual_captcha_resolution(session: dict[str, Any], product_id: str) -> tuple[str, float]:
    ...
    print('请在浏览器中手动点击 “I am human”。')
    print("脚本正在等待验证通过，不需要在终端按 Enter。")
    print("验证码通过后将自动重新检测当前页面状态，并重试当前商品。")
    ...
    while True:
        ...
        if contains_login_page(current_url, html_text, visible_text):
            return "login_required", elapsed_seconds
        body_signal_count = count_product_body_signals(html_text, visible_text) if html_text else 0
        if not is_challenge_url(current_url) and body_signal_count >= 2:
...
def main() -> None:
    """Run the V2.9D-1B CloakBrowser smoke test with the KDL proxy."""
    ensure_dirs()
    ...
    print("V2.9D-1B CloakBrowser Smoke Test 启动")
    print("=" * 64)
    print(f"CLOAKBROWSER_ENABLED: {CLOAKBROWSER_ENABLED}")
    print(f"CLOAK_HUMANIZE_ENABLED: {CLOAK_HUMANIZE_ENABLED}")
    print(f"CLOAK_GEOIP_ENABLED: {CLOAK_GEOIP_ENABLED}")
    print(f"LOW_RISK_SAMPLE_MODE: {LOW_RISK_SAMPLE_MODE}")
    print(f"LOW_RISK_SKIP_TOP_N: {LOW_RISK_SKIP_TOP_N}")
    print(f"LOW_RISK_SAMPLE_LIMIT: {LOW_RISK_SAMPLE_LIMIT}")
    print(f"BROWSERLEAKS_BEFORE_COLLECTION_ENABLED: {BROWSERLEAKS_BEFORE_COLLECTION_ENABLED}")
    print(f"BROWSERLEAKS_OBSERVE_SECONDS: {BROWSERLEAKS_OBSERVE_SECONDS}")
    print(f"CAPTCHA_MANUAL_RESOLVE_ENABLED: {CAPTCHA_MANUAL_RESOLVE_ENABLED}")
    print(f"CAPTCHA_MANUAL_RESOLVE_TIMEOUT_SECONDS: {CAPTCHA_MANUAL_RESOLVE_TIMEOUT_SECONDS}")
    print(f"CAPTCHA_MANUAL_POLL_INTERVAL_SECONDS: {CAPTCHA_MANUAL_POLL_INTERVAL_SECONDS}")
    print("session_type: cloak_local_proxy")
    ...
    if not PROXY_CONFIGURED:
        ...
        if CLOAKBROWSER_ENABLED:
            try:
                print("开始创建 cloak_local_proxy session...", flush=True)
                session = create_cloak_proxy_browser_session()
                print("cloak_local_proxy session 创建完成，准备执行代理出口预检。", flush=True)
                print("开始执行 KDL 代理出口预检：api.ipify.org", flush=True)
                proxy_preflight_status, proxy_preflight_reason = run_proxy_preflight(session)
                print(
                    f"KDL 代理出口预检结果：{proxy_preflight_status}，原因：{proxy_preflight_reason}",
                    flush=True,
                )
                ...
                if proxy_preflight_status != "success":
                    ...
                    for index, product in enumerate(selected_products, start=1):
                        ...
                        if is_captcha_status(first_row) and CAPTCHA_MANUAL_RESOLVE_ENABLED:
                            ...
                            if captcha_wait_status == "resolved":
                                recovery_used = True
                                print("人工验证码已通过，开始 stable 模式重采当前商品。")
                                ...
                                print("当前会话已不适合继续采集。")
                                print("本轮采集立即停止。")
                            elif captcha_wait_status == "timeout":
                                final_fetch_mode = "captcha_manual_timeout"
                                print("人工验证码等待超时，当前商品记录为 blocked_or_verify。")
                        elif (
                        ...
                        row["captcha_recovery_success"] = str(
                            captcha_wait_status == "resolved" and is_success_row(row)
                        )
                        ...
                        if (
                            captcha_wait_status == "resolved"
                            and is_captcha_status(row)
                            and consecutive_blocked_products < MAX_CONSECUTIVE_BLOCKED_PRODUCTS
                        ):
                            print("人工验证码通过后重采仍进入 risk/challenge，已记录，继续测试下一个商品。")
                        if (
                            is_captcha_status(row)
                            and not CAPTCHA_MANUAL_RESOLVE_ENABLED
                            and consecutive_blocked_products < MAX_CONSECUTIVE_BLOCKED_PRODUCTS
                        ):
                            print("当前商品进入 risk/challenge，已记录，继续测试下一个商品。")
