"""V2.9D-1A：browser.ws 远程 CDP fast_mode 降级保护测试版。
...
OUTPUT_CSV_PATH = PROJECT_ROOT / "data" / "output" / "v2_9d1a_50_sold_products_detail_attributes_test.csv"
SUMMARY_CSV_PATH = PROJECT_ROOT / "data" / "output" / "v2_9d1a_50_sold_products_detail_summary_test.csv"
DEBUG_DIR = PROJECT_ROOT / "data" / "debug" / "v2_9d1a_50_sold_products_test"
MAX_PRODUCTS = 50
...
REMOTE_FAST_MIN_PAGE_SECONDS = 14
REMOTE_FAST_MIN_ATTRIBUTE_COUNT = 5
REMOTE_FAST_EMPTY_DETAIL_STABLE_COOLDOWN = 3
...
def smart_wait_detail_ready(browser_page: Any) -> None:
    ...
    browser_page.wait_for_timeout(1000)
    if REMOTE_CDP_ENABLED:
        min_page_seconds = REMOTE_FAST_MIN_PAGE_SECONDS
        min_attribute_count = REMOTE_FAST_MIN_ATTRIBUTE_COUNT
    else:
    ...
    for _ in range(FAST_MODE_MAX_READY_LOOPS):
        ...
        if elapsed_seconds >= min_page_seconds:
            ...
            if body_signal_count >= 3 and attribute_count > 0:
                return
            if not REMOTE_CDP_ENABLED and body_signal_count >= 4:
                return
...
def main() -> None:
    """执行 V2.9D-1A 远程 fast_mode 降级保护测试。"""
    ensure_dirs()
    ...
    print("V2.9D-1A 测试版：browser.ws 远程 CDP fast_mode 降级保护验证")
    print("=" * 64)
        ...
        try:
            for index, product in enumerate(selected_products, start=1):
                ...
                if stable_cooldown_remaining > 0:
                    ...
                    print(
                        "stable cooldown 生效："
                        f"当前商品使用 stable_mode，剩余 cooldown 商品数={stable_cooldown_remaining}"
                    )
                else:
                ...
                if (
                    ...
                    stable_cooldown_remaining = REMOTE_FAST_EMPTY_DETAIL_STABLE_COOLDOWN
                    print("fast_mode 空详情率偏高，进入 stable cooldown。")
                    print(
                        f"后续 {REMOTE_FAST_EMPTY_DETAIL_STABLE_COOLDOWN} "
                        "个商品将优先使用 stable_mode。"
                    )
                ...
                print(
                    ...
                    f"stable_cooldown_remaining: {stable_cooldown_remaining}"
                )
