...
DEBUG_HTML_PATH = PROJECT_ROOT / "data" / "debug" / "v2_2a_page.html"
CSV_PATH = PROJECT_ROOT / "data" / "output" / "v2_2a_products_page_1.csv"
CSV_FIELDS = [
    "title",
    "price",
    "product_url",
...
def main() -> None:
    ...
    print("页面状态识别信号：")
    for signal_name, is_hit in status_signals.items():
        print(f"- {signal_name}：{is_hit}")

    if not products:
        raise RuntimeError("未采集到商品数据，请检查 data/debug/v2_2a_page.html 中的页面结构或拦截状态。")
